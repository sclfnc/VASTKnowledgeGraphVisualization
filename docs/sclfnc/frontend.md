...
panels, Lucide for icons, `@vueform/slider` for range controls.

For setup (`npm` scripts, Node version), see [`frontend/README.md`](../../frontend/README.md). The
cross-panel interaction contract is in [contract.md](contract.md); the D3 panel system in
[panels.md](panels.md).
...
└── panels/                   # D3 viz panels (see panels.md)
```
...
- **Type colours are shared and effective-aware** — `useNodeTypeColors` / `useEdgeTypeColors`. Never a local `d3.scaleOrdinal` for types (same hue per type across all panels).
